...
(defn api [store mimi db-conn user-store payment-gateway]
...
(s/defrecord ApiComponent [store mimi db-conn user-store payment-gateway]
  component/Lifecycle
  (start [component]
    (assoc component :routes (api store mimi db-conn user-store payment-gateway)))
  (stop [component]
        component))
